...
/**
 * Servlet implementation class CV_decrypted
 */
@WebServlet("/CV_decrypted")
public class CV_decrypted extends HttpServlet {
		...
		String sec= request.getParameter("sec");
		 try {
			 if(sec.equalsIgnoreCase("low"))
		       ...
		       while(rs1.next()) {
			   String qur="insert into cv_dec values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	           ...
	           ps.executeUpdate();
		        ...
		        out.print("<html><body><script>alert('Low Level Security Data Decrypted Successfully')</script></body></html>");
		 ...
		 else if(sec.equalsIgnoreCase("medium")) 
			 {
		      ...
		      PrintWriter out = response.getWriter();			
		      out.print("<html><body><script>alert('Medium Level Security Data Decrypted Successfully ')</script></body></html>");
		      RequestDispatcher rd = request.getRequestDispatcher("CV_homepg.html");
		      rd.include(request,response);
    }
		 ...
		 else if(sec.equalsIgnoreCase("high")) 
		  {
		   ...
		   PrintWriter out = response.getWriter();			
		   out.print("<html><body><script>alert('High Level Security Data Decrypted Successfully ')</script></body></html>");
		   RequestDispatcher rd = request.getRequestDispatcher("CV_homepg.html");
		  rd.include(request,response);
		 }
  }       catch (Exception e)     
	      {
		   PrintWriter out = response.getWriter();
	       out.print("<html><body><script>alert(' not decrypted')</script></body></html>");
		   RequestDispatcher rd = request.getRequestDispatcher("CV_homepg.html");
		   rd.include(request,response);
